...
export function RecruitmentKanbanBoard({ candidates }: { candidates: ATSCandidate[] }) {
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={onDragStart} onDragEnd={onDragEnd}>
      ...
      <DragOverlay>
        {activeCandidate ? (
          <div className="w-64 rounded-lg border border-slate-200 bg-white p-3 shadow-soft">
...
function KanbanColumn({ stage, candidates }: { stage: Stage; candidates: ATSCandidate[] }) {
  ...
  return (
    <section
      ref={setNodeRef}
      className={cn(
        "flex max-h-full w-64 shrink-0 flex-col rounded-lg border border-slate-200 bg-slate-100/80",
        isOver && "ring-2 ring-emerald-400"
      )}
    >
      <header className="flex items-center justify-between px-3 py-3">
        <h3 className="text-sm font-semibold text-slate-800">{STAGE_LABELS[stage]}</h3>
        <span className="rounded-full bg-white px-2 py-0.5 text-xs font-medium text-slate-500">{candidates.length}</span>
      </header>
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2">
        <button
          className="mt-0.5 cursor-grab rounded-md p-1 text-slate-300 hover:bg-slate-100 hover:text-slate-600"
          type="button"
          {...listeners}
          {...attributes}
        >
        ...
        <div className="min-w-0 flex-1">
          ...
          {(candidate as any).job && typeof (candidate as any).job === "object" && (candidate as any).job.title ? (
            <p className="mt-0.5 text-xs text-slate-400">{(candidate as any).job.title}</p>
          ) : null}
        </div>
      </div>
