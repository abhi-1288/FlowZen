...
export function OnboardingTab({
  ...
  const [revokingItCodeId, setRevokingItCodeId] = useState<string | null>(null);
  const [itCodeExpiryHours, setItCodeExpiryHours] = useState(24);
  const [itCodeMaxUses, setItCodeMaxUses] = useState(1);

  async function fetchItCodes() {
    try {
      setItCodesLoading(true);
      const res = await apiFetch<{ codes: AnyRecord[] }>("/api/it/codes", undefined, { toast: false });
      setItCodes(Array.isArray(res?.codes) ? res.codes : []);
    } catch {
      setItCodes([]);
    } finally {
      setItCodesLoading(false);
    }
  }

  async function generateItCode() {
    if (generatingItCode) return;
    setGeneratingItCode(true);
    try {
      const res = await apiFetch<{ code: AnyRecord }>("/api/it/codes", {
        method: "POST",
        body: JSON.stringify({ expiresInHours: itCodeExpiryHours, maxUses: itCodeMaxUses }),
      });
      const codeValue = String(res?.code?.code ?? "");
      showToast(codeValue ? `Joining code ${codeValue} generated.` : "Joining code generated.");
      await fetchItCodes();
    } catch (err) {
      showToast(err instanceof Error ? err.message : "Unable to generate joining code.", "error");
    } finally {
      setGeneratingItCode(false);
    }
  }

  async function revokeItCode(codeId: string) {
    if (revokingItCodeId) return;
    setRevokingItCodeId(codeId);
    try {
      await apiFetch(`/api/it/codes/${codeId}`, { method: "DELETE" });
      showToast("Joining code revoked.");
      await fetchItCodes();
    } catch (err) {
      showToast(err instanceof Error ? err.message : "Unable to revoke joining code.", "error");
    } finally {
      setRevokingItCodeId(null);
    }
  }

  const copyItCode = (value: string, label = "Joining code") => {
    navigator.clipboard.writeText(value);
    showToast(`${label} copied.`);
  };

  const itJoinUrl = (value: string) =>
    typeof window !== "undefined" ? `${window.location.origin}/join?code=${value}` : value;

  useEffect(() => { void fetchItCodes(); }, []);
  ...
  async function joinItTeam(event: FormEvent) {
    event.preventDefault();
    setJoinLoading(true);
    try {
      const normalized = String(companyCode ?? "").trim().toUpperCase();
      if (!normalized.startsWith("IT-")) {
        showToast("IT Support joins require an IT- joining code generated by an IT Admin.", "error");
        return;
      }
      const data = await apiFetch<{ creatorName?: string }>("/api/it/join", { method: "POST", body: JSON.stringify({ code: normalized }) });
      showToast(data?.creatorName ? `Join request sent to ${data.creatorName} for approval.` : "Join request sent to the IT Admin for approval.");
      await refresh();
    } catch (err) {
      showToast(err instanceof Error ? err.message : "Unable to send join request.", "error");
    } finally {
    ...
    <div className="grid gap-5 xl:grid-cols-2">
          ...
          <>
            <section className={sectionBase}>
              <div className="mb-5 border-l-4 border-indigo-500 pl-4">
                <h3 className="text-base font-semibold text-slate-900">IT Administration</h3>
                <p className="mt-0.5 text-sm text-slate-500">Generate joining codes for IT Administration staff and manage active invitations.</p>
              </div>
              <div className="mb-4 flex flex-wrap items-end gap-3">
                <div className="flex items-center gap-2">
                  <label className="text-xs font-medium text-slate-500" htmlFor="it-expiry">Expires in</label>
                  <input id="it-expiry" type="number" min={1} max={168} value={itCodeExpiryHours}
                    onChange={(e) => setItCodeExpiryHours(Math.max(1, Number(e.target.value) || 1))}
                    className="w-20 rounded-md neu-inset px-2 py-1.5 text-xs" />
                  <span className="text-xs text-slate-500">hrs</span>
                </div>
                <div className="flex items-center gap-2">
                  <label className="text-xs font-medium text-slate-500" htmlFor="it-uses">Uses</label>
                  <input id="it-uses" type="number" min={1} max={100} value={itCodeMaxUses}
                    onChange={(e) => setItCodeMaxUses(Math.max(1, Number(e.target.value) || 1))}
                    className="w-20 rounded-md neu-inset px-2 py-1.5 text-xs" />
                </div>
                <ActionButton variant="primary" disabled={generatingItCode} onClick={async () => { await generateItCode(); }} type="button">
                  {generatingItCode ? "Generating..." : "+ Generate Joining Code"}
                </ActionButton>
              </div>

              <div className="space-y-3">
                {itCodesLoading ? (
                  <p className="text-sm text-slate-500">Loading joining codes...</p>
                ) : itCodes.length === 0 ? (
                  <p className="rounded-lg bg-[var(--c-bg-muted)] px-3 py-4 text-sm text-slate-500">No joining codes yet. Generate one above to invite IT Administration staff.</p>
                ) : (
                  itCodes.map((item: AnyRecord) => {
                    const cid = String(item.id ?? "");
                    const codeVal = String(item.code ?? "");
                    const expiresAt = item.expiresAt ? new Date(String(item.expiresAt)) : null;
                    const maxUses = Number(item.maxUses ?? 1);
                    const usedCount = Number(item.usedCount ?? 0);
                    const isExpired = !expiresAt || expiresAt < new Date() || String(item.status ?? "") === "expired";
                    const isRevoked = String(item.status ?? "") === "revoked";
                    const remaining = isExpired || isRevoked ? 0 : Math.max(0, maxUses - usedCount);
                    return (
                      <div key={cid} className={`rounded-lg border p-4 ${isExpired || isRevoked ? "border-slate-200 bg-slate-50 opacity-70" : "border-dashed border-slate-300 bg-[var(--c-bg-muted)]"}`}>
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          <div className="min-w-0">
                            <p className="font-mono text-sm font-semibold text-indigo-700">{codeVal}</p>
                            <p className="mt-0.5 text-xs text-slate-500">
                              {isRevoked ? "Revoked" : isExpired ? "Expired" : `Expires ${expiresAt ? expiresAt.toLocaleString() : "-"} · ${Math.max(0, remaining)} use${remaining === 1 ? "" : "s"} left`}
                            </p>
                          </div>
                          <div className="flex flex-wrap gap-2">
                            <ActionButton variant="secondary" className="px-3" type="button" disabled={isExpired || isRevoked} onClick={() => copyItCode(codeVal)}>Copy</ActionButton>
                            <ActionButton variant="secondary" className="px-3" type="button" disabled={isExpired || isRevoked} onClick={() => { navigator.clipboard.writeText(itJoinUrl(codeVal)); showToast("Join URL copied."); }}>Copy URL</ActionButton>
                            <ActionButton variant="danger" className="px-3" type="button" disabled={isRevoked || revokingItCodeId === cid} onClick={() => revokeItCode(cid)}>
                              {revokingItCodeId === cid ? "Revoking..." : "Revoke"}
                            </ActionButton>
                          </div>
                        </div>
                      </div>
      ...
      {role === "it-administration" && company?.status !== "taken-down" ? (
        profile?.companyStatus === "approved" ? (
          ...
          <JoinPanel title="Join company as IT Support" placeholder="IT joining code (IT-...)" value={companyCode} onChange={setCompanyCode} onSubmit={joinItTeam}
            loading={joinLoading} status={profile?.companyStatus ? String(profile.companyStatus) : undefined} onCancelRequest={() => setCancelJoinModal(true)} />
        )
      ) : null}
