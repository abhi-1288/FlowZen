    ...
    <Card>
      ...
      <div className="mt-3 space-y-2">
          ...
          <>
            {policyData.foodAmount > 0 ? (
              <div className="flex items-center justify-between rounded-lg neu-inset px-4 py-3">
                ...
                <div className="flex gap-2">
                  <button className={`rounded-full px-3 py-1.5 text-xs font-medium ${foodOptedIn ? "neu-tab-pressed" : "neu-tab-raised"}`} onClick={() => onToggleOptInOut("food", true)}>Active</button>
                  <button className={`rounded-full px-3 py-1.5 text-xs font-medium ${!foodOptedIn ? "neu-tab-pressed" : "neu-tab-raised"}`} onClick={() => onToggleOptInOut("food", false)}>Opt Out</button>
                </div>
              </div>
            ) : null}
            {policyData.travelAccommodationAmount > 0 ? (
              <div className="flex items-center justify-between rounded-lg neu-inset px-4 py-3">
                ...
                <div className="flex gap-2">
                  <button className={`rounded-full px-3 py-1.5 text-xs font-medium ${travelOptedIn ? "neu-tab-pressed" : "neu-tab-raised"}`} onClick={() => onToggleOptInOut("travel", true)}>Active</button>
                  <button className={`rounded-full px-3 py-1.5 text-xs font-medium ${!travelOptedIn ? "neu-tab-pressed" : "neu-tab-raised"}`} onClick={() => onToggleOptInOut("travel", false)}>Opt Out</button>
                </div>
              </div>
            ) : null}
          </>
        ...
        {canConfigurePercentages ? (
          <div className="rounded-lg neu-inset px-4 py-3 space-y-2">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <input className="w-full rounded-lg border border-[var(--c-border-light)] px-3 py-2 text-sm" type="number" min="0" max="100" step="0.01" value={pfPctInput} onChange={(e) => onPfPctChange?.(e.target.value)} />
              </div>
              <div>
                <label className="text-xs font-medium text-slate-500 mb-1 block">ESIC (%)</label>
                <input className="w-full rounded-lg border border-[var(--c-border-light)] px-3 py-2 text-sm" type="number" min="0" max="100" step="0.01" value={esicPctInput} onChange={(e) => onEsicPctChange?.(e.target.value)} />
              </div>
              <div>
                <label className="text-xs font-medium text-slate-500 mb-1 block">TDS (%)</label>
                <input className="w-full rounded-lg border border-[var(--c-border-light)] px-3 py-2 text-sm" type="number" min="0" max="100" step="0.01" value={tdsPctInput} onChange={(e) => onTdsPctChange?.(e.target.value)} />
              </div>
            </div>
            <div className="flex justify-end">
              <button className="rounded-full neu-tab-pressed px-4 py-1.5 text-xs font-medium" onClick={onSavePercentages}>Save Percentages</button>
            </div>
          </div>
        ) : policyData ? (
          <div className="flex items-center justify-between rounded-lg neu-inset px-4 py-3">
        ...
        {policyData ? (
          <div className="flex items-center justify-between rounded-lg neu-inset px-4 py-3">
            ...
            {canConfigure ? (
              <div className="flex gap-2">
                <button
                  className={`rounded-full px-3 py-1.5 text-xs font-medium ${policyData.advanceSalaryEnabled ? "neu-tab-pressed" : "neu-tab-raised"}`}
              ...
              <span className={`rounded-lg px-3 py-1.5 text-xs font-medium ${policyData.advanceSalaryEnabled ? "bg-emerald-50 text-emerald-700" : "bg-[var(--c-bg-muted)] text-slate-500"}`}>
