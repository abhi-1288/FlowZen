...
const overlayClass = "fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4";
const modalClass = "w-full max-w-lg overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl";
    ...
    <div className={overlayClass} role="presentation" onClick={(e) => { if (e.target === e.currentTarget) onCancel(); }}>
      <div className={modalClass} role="dialog" aria-modal="true" aria-labelledby="fire-modal-title">
        <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-6 py-4">
          <div>
            <h4 className="text-lg font-semibold text-rose-700" id="fire-modal-title">Fire member</h4>
          ...
          <button
            aria-label="Close"
            className="inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-all duration-200 disabled:cursor-not-allowed disabled:opacity-50 text-slate-500 hover:text-slate-700 hover:bg-slate-50 h-10 w-10"
            type="button"
            onClick={onCancel}
          >
            <X size={18} />
