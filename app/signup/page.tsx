      ...
      <section className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-8 shadow-soft">
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Verify your email with a 6-digit OTP.</p>
          </div>
        </div>
          ...
          <>
            <form className="space-y-4" onSubmit={register}>
              ...
              <label className="block">
                ...
                <select className="w-full rounded-lg border border-slate-200 px-3 py-2.5 outline-none ring-emerald-500 focus:ring-2" value={role} onChange={(event) => setRole(event.target.value as UserRole)}>
...
function Field({ label, value, onChange, type = "text", inputMode, pattern }: { label: string; value: string; onChange: (value: string) => void; type?: string; inputMode?: "numeric"; pattern?: string }) {
  ...
  return (
    ...
    ">
      ...
      <input className={`w-full rounded-lg border border-slate-200 px-3 py-2.5 outline-none ring-emerald-500 focus:ring-2${isPassword ? "pr-10" : ""}`} type={isPassword && showPassword ? "text" : type} value={value} onChange={(event) => onChange(event.target.value)} minLength={type === "password" ? 8 : undefined} inputMode={inputMode} pattern={pattern} required />
...
function Notice({ error, notice }: { error: string; notice: string }) {
  ...
  if (notice) return <p className="rounded-lg bg-emerald-50 px-3 py-2 text-sm text-emerald-700">{notice}</p>;
  return null;
}
